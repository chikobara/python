...
ASPECT = 21.0 / 10.0  # A 45 degree line from (1,1) --> (23, 10)
ASPECT = 15.0 / 10.0  # A 45 degree line from (1,1) --> (23, 10)
...
if __name__ == "__main__":
    ...
    if star:
        ...
        screen = Screen(w, h)
        start = 0
        x, y = nearest(w / 2.0) - 1, nearest(h / 2.0) - 1
        oldx, oldy = w - 1, y
        for ang in range(start, 360 + start + 10, 10):
            x2, y2 = nearest(x + w / 2.0 * cos(ang * pi / 180.0)), nearest(
                y + h / 2.0 * sin(ang * pi / 180.0)
            )
    ...
    if square:
        ...
        start = 30
        x, y = nearest(w / 2.0) - 1, nearest(h / 2.0) - 1
        for i, size in enumerate([w / 2.0, w / 2.0 - 4]):
            x2, y2 = nearest(x + size * cos(start * pi / 180.0)), nearest(
                y + (size / ASPECT) * sin(start * pi / 180.0)
            )
            oldx, oldy = x2, y2
            for ang in range(start, 360 + start + 60, 60):
                x2, y2 = nearest(x + size * cos(ang * pi / 180.0)), nearest(
                    y + (size / ASPECT) * sin(ang * pi / 180.0)
                )
    ...
    if normal:
        ...
        ##        smiles = "C"*25
        output, tmp = subprocess.Popen(
            ["obabel", "-:%s" % smiles, "-opaint", "-xM"],
        ...
        for line in output.split("\n"):
            ...
            if line.startswith("DrawLine"):
                ...
                screen.text(
                    broken[3][1:-1], (nearest(vals[0]), nearest(vals[1] / ASPECT))
                )
